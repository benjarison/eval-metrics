...
impl std::fmt::Display for BinaryConfusionMatrix {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "{}",
            display::stringify_confusion_matrix(&counts, &outcomes)
        )
    }
}
...
impl MultiConfusionMatrix {
    ...
    pub fn rk(&self) -> Result<f64, EvalError> {
        ...
        let tp = t
            .iter()
            .zip(p)
            .fold(0.0, |acc, (t_val, p_val)| acc + t_val * p_val);
        let num = c * s - tp;
    ...
    fn macro_metric(&self, pcm: &Vec<Result<f64, EvalError>>) -> Result<f64, EvalError> {
        pcm.iter()
            .try_fold(0.0, |sum, metric| match metric {
                Ok(m) => Ok(sum + m),
                Err(e) => Err(e.clone()),
            })
    ...
    fn weighted_metric(&self, pcm: &Vec<Result<f64, EvalError>>) -> Result<f64, EvalError> {
        pcm.iter()
            ...
            .try_fold(0.0, |val, (metric, &class)| match metric {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if self.dim <= 25 {
            ...
            write!(
                f,
                "{}",
                display::stringify_confusion_matrix(&self.counts, &outcomes)
            )
        } else {
...
fn create_pairs<T: Scalar>(
    scores: &Vec<T>,
    labels: &Vec<bool>,
) -> Result<(Vec<(T, bool)>, usize), EvalError> {
    let n = scores.len();
...
mod tests {
    ...
    fn test_binary_confusion_matrix_empty() {
        assert!(
            BinaryConfusionMatrix::compute(&Vec::<f64>::new(), &Vec::<bool>::new(), 0.5).is_err()
        );
    }
    ...
    fn test_binary_confusion_matrix_unequal_length() {
        assert!(
            BinaryConfusionMatrix::compute(&vec![0.1, 0.2], &vec![true, false, true], 0.5).is_err()
        );
    }
    ...
    fn test_binary_f1_0p_0r() {
        ...
        assert_eq!(
            BinaryConfusionMatrix::compute(&scores, &labels, 0.5)
                .unwrap()
                .f1()
                .unwrap(),
            0.0
    ...
    fn test_auc_tied_scores() {
        ...
        assert_approx_eq!(
            RocCurve::compute(&scores2, &labels4).unwrap().auc(),
            0.6904761904761905
        );
    }
    ...
    fn test_ap() {
        let (scores, labels) = binary_data();
        assert_approx_eq!(
            PrCurve::compute(&scores, &labels).unwrap().ap(),
            0.6805555555555556
        );
        ...
        let labels2 = vec![false, true, false, true];
        assert_approx_eq!(
            PrCurve::compute(&scores2, &labels2).unwrap().ap(),
            0.58333333333333
        );
    }
    ...
    fn test_ap_tied_scores() {
        ...
        let labels3 = vec![false, false, false, true, true, false, true];
        assert_approx_eq!(
            PrCurve::compute(&scores, &labels1).unwrap().ap(),
            0.7333333333333
        );
        assert_approx_eq!(
            PrCurve::compute(&scores, &labels2).unwrap().ap(),
            0.7333333333333
        );
        assert_approx_eq!(
            PrCurve::compute(&scores, &labels3).unwrap().ap(),
            0.7333333333333
        );
    }
    ...
    fn test_multi_confusion_matrix() {
        ...
        assert_eq!(
            matrix.counts,
            vec![vec![1, 1, 1], vec![1, 1, 0], vec![0, 0, 2]]
        );
    ...
    fn test_multi_confusion_matrix_unequal_length() {
        assert!(MultiConfusionMatrix::compute(
            &vec![vec![0.2, 0.4, 0.4], vec![0.5, 0.1, 0.4]],
            &vec![2, 1, 0]
        )
        .is_err());
    }
    ...
    fn test_multi_confusion_matrix_counts() {
        ...
        assert_eq!(
            matrix.counts,
            vec![vec![6, 3, 1], vec![4, 2, 7], vec![5, 2, 8]]
        );
    }
    ...
    fn test_multi_precision() {
        ...
        let matrix = MultiConfusionMatrix::compute(&scores, &labels).unwrap();
        assert_approx_eq!(
            matrix.precision(&Averaging::Macro).unwrap(),
            0.611111111111111
        );
    ...
    fn test_multi_recall() {
        ...
        let matrix = MultiConfusionMatrix::compute(&scores, &labels).unwrap();
        assert_approx_eq!(
            matrix.recall(&Averaging::Macro).unwrap(),
            0.5555555555555555
        );
        assert_approx_eq!(
            matrix.recall(&Averaging::Weighted).unwrap(),
            0.5714285714285714
        );
    ...
    fn test_multi_f1_0p_0r() {
        ...
        assert_eq!(
            MultiConfusionMatrix::compute(&scores, &labels)
                .unwrap()
                .f1(&Averaging::Macro)
                .unwrap(),
            0.0
